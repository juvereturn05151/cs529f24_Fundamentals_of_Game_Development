...
void Scene::handleCollision()
{
    if (character1->getHurtBox()->isColliding(*character2->getHurtBox())) 
    {
        ...
        Vector3 displacement = character1->getTransform()->getPosition() - character2->getTransform()->getPosition();
        displacement.normalized();

        // Define a pushback distance
        float pushbackForce = 50.0f;

        character1->GetPhysicsComp()->applyForce((displacement * -1)* pushbackForce);
        character2->GetPhysicsComp()->applyForce(displacement * pushbackForce);
    }
}
